...
void bmSavePatch(uint8_t slot){
  ...
  p.crush=bmStoredDrive; p.crop=128; p.drop=0;   // crop/drop bytes vestigial (filter & chance not persisted)
  p.accent=bmStoredAccent; p.pitch=bmStoredPitch;
  p.chance=bmFillMode; p.zoom=bmFillType;
  p.range=bmStoredSwing; p.humanize=bmStoredHumanize;
...
void bmLoadPatch(uint8_t slot){
  ...
  bmStoredBeat=p.beat; bmStoredDrive=p.crush;   // p.crop ignored — filter is a live control
  bmStoredAccent=p.accent; bmStoredPitch=p.pitch;
  bmFillMode=(p.chance<=3)?p.chance:0; bmFillType=(p.zoom<=3)?p.zoom:0;
  bmStoredSwing=p.range; bmStoredHumanize=p.humanize;
      ...
      if(r){
        ...
        if(bmPChord[i]&&(i==0||i==1)&&bmPChord[0]&&bmPChord[1]){
          ...
          if(holdMs>=1000){ bmDoReset(); }
            ...
            if(bmPlaying){bmPulseNum=0;bmStepNum=0;bmDrumPos=0;bmAbsStep=0;bmFillTouched=0;bmWasInFill=false;bmFillDblPending=false;bmSwingPending=false;for(byte _h=0;_h<NUM_DRUMS;_h++)bmHitPend[_h]=false;}
            bmPlayChanged=true;
