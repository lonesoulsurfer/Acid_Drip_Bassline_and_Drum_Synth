...
// Extracted from bmFillDrumBufferTo() so it can be called from core 0
// (GP15/updateAudio) in a later stage. Returns one sample (post echo/
// verb/softclip/DRIFT output-trim). Reads the ch2* globals; the echo/
// verb position statics inside belong to whichever SINGLE core calls
// it. Today that is core 0 (updateAudio/GP15). If DRIFT is ever routed
// to GP2 instead, core 1 owns them — see the ownership note above.
// __not_in_flash_func: this is now audio-critical on BOTH cores, and on the
// GP2 route it runs on core 1 — the same core that owns all TFT/SPI. A
