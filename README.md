...
| Sync in | GP2 (hardware SPDT switch selects drum/DRIFT audio OR sync input) |
...
GP2 is shared between drum audio output (in V5, also the DRIFT synth voice, which sums with the drums on the same output) and external sync clock input. A physical **SPDT switch on the PCB** routes the pin to either function.

**Boot detection selects the mode:**

- **Drum mode (default):** GP2 is claimed by PWMAudio DMA and outputs drum (and, in V5, DRIFT) audio. Switch in the DRUMS position.
...
> The switch position and held pad must match. If the switch is in SYNC position but the device boots into drum mode, GP2 will try to drive audio into the sync signal line. In V5, Sync In mode silences drums and DRIFT together, since both share GP2; the acid voice on GP15 is unaffected either way.
