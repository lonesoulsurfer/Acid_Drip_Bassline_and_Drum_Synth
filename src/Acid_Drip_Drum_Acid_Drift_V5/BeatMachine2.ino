...
// Whether drums were playing when this slot was saved, packed into the
// spare bit of p.fillMode (fillMode only ever uses values 0-3 — see the
// "<=3" clamp in bmLoadPatch()). Deliberately NOT a new struct field: this
// struct sits right up against CH2X_ADDR with ZERO slack (see the comment
// on BmPatch below — growing it by even 1 byte was already tried once,
// pushed BM_SLOT_ADDR(3) past 400 into DRIFT's territory, and was
// reverted). Not repeating that mistake for one boolean.
#define BM_PATCH_PLAYING_BIT 0x04
...
void bmSavePatch(uint8_t slot){
  ...
  p.fillMode=bmFillMode | (bmPlaying?BM_PATCH_PLAYING_BIT:0); p.fillType=bmFillType;
...
void bmLoadPatch(uint8_t slot){
  ...
  bmFillMode=p.fillMode & 0x03; bmFillType=(p.fillType<=3)?p.fillType:0;
  // p.fillMode's spare bit (BM_PATCH_PLAYING_BIT) carries whether drums were
  // playing at save time. Deliberately NOT applied here — see
  // bmPatchWasPlaying(), used only by loadAllFromSlot() (Acid_Drip_V5.ino).
...
// Were drums playing when this slot was saved? Used only by
// loadAllFromSlot() (Acid_Drip_V5.ino) to force play state.
bool bmPatchWasPlaying(uint8_t slot){
  if(slot>=BM_NUM_SLOTS||!bmSlotHasData[slot]) return false;
  BmPatch p; EEPROM.get(BM_SLOT_ADDR(slot),p);
  if(p.valid!=BM_PATCH_VALID) return false;
  return (p.fillMode & BM_PATCH_PLAYING_BIT) != 0;
}
...
// ── Stop drums cleanly ───────────────────────────────────────────────
// Mirrors the inline STOP branch in the drum-mode pad handler below.
// Named/exposed the same way as bmStartDrums() so loadAllFromSlot()
// (Acid_Drip_V5.ino) can force drums off without needing its own copy
// of the dirty-flag bookkeeping.
void bmStopDrums(){
  bmPlaying=false;
  bmPlayChanged=true;
  bmVsDirty=true;bmInfoDirty=true;bmGridDirty=true;bmPadsDirty=true;
  ...
  for(byte i=0;i<16;i++){
    ...
    if(r!=bmPLast[i]&&(now-bmPDeb[i])>20){
      ...
      if(r){
        ...
        // Gated on !bmFuncMode: pad 16 is ALSO the FUNC-slot-8 (FILL)
        // selector — see the press handler above, which sets bmFuncSel on
        // press when bmFuncMode is true. Without this guard, releasing
        // pad 16 after selecting FILL fell through to the "not editing"
        // tier-0 case below and forced CUSTOM edit mode open, silently
        // overriding the FILL selection that had just been made.
        if(i==BM_CUSTOM_IDX && !bmPChord[i] && !bmFuncMode){
          uint32_t holdMs=now-bmPDown[i];
...
// bmRenderDrumSample: render ONE drum-bus sample — all 8 voices + envelopes
// advanced, drive saturator, bipolar DJ filter, per-bus soft-clip and the
// MIX EDIT drum trim. Returns the drum-only mix (DRIFT is summed separately
// by each caller). ADVANCES voice/filter/envelope state, so it must be called
// exactly once per output sample and from ONE core only. Extracted verbatim
// from bmFillDrumBufferTo()'s loop so the GP2 path (normal mode) and the GP15
// fold-in (sync mode, updateAudio()) render bit-identically. __not_in_flash so
// the sync-mode caller on core 0's audio hot path takes no flash-fetch stall.
int32_t __not_in_flash_func(bmRenderDrumSample)(){
    int32_t mix = 0;
    if(bmPlaying){
...
// ---- core 1 (bmFillDrumBufferTo / GP2 / drift+drums) — written ONLY on core 1 ----
// This path is a BURST renderer (loop1() tops the DMA buffer up in chunks,
// not one sample per call), so "per-call" time isn't meaningful on its own —
// what's tracked is total render time and total samples rendered per
// ~1-second window, giving a real average us/sample cost, plus the worst
// single burst duration. NOTE: burst duration will spike hard (hundreds of
// ms) during bmFillDrumBufferMax() pre-fills (EEPROM commits, mode
// switches) — that's expected and NOT a sign of a problem; read
// bench1_maxBurstUsOut with that in mind, not as a steady-state number.
static uint32_t bench1_windowStart     = 0;
static uint32_t bench1_samplesInWindow = 0;
static uint32_t bench1_sumUsInWindow   = 0;
static uint32_t bench1_maxBurstUs      = 0;

volatile uint32_t bench1_avgUsPerSample = 0;  // read by loop1()'s reporter (main .ino)
volatile uint32_t bench1_maxBurstUsOut  = 0;
volatile uint32_t bench1_samplesPerSec  = 0;
#endif
...
#if BENCH_AUDIO_HEADROOM
  uint32_t __burstT0 = time_us_32();
  uint32_t __burstSamples = 0;
#endif

  while(bmPWM.availableForWrite() > (int32_t)(BM_BUF_CAPACITY - targetQueued)){
    int32_t mix = bmRenderDrumSample();   // drums (extracted; see bmRenderDrumSample() above)
...
#if BENCH_AUDIO_HEADROOM
  if (__burstSamples) {
    uint32_t burstUs = time_us_32() - __burstT0;
    if (burstUs > bench1_maxBurstUs) bench1_maxBurstUs = burstUs;
    bench1_sumUsInWindow   += burstUs;
    bench1_samplesInWindow += __burstSamples;
    if (bench1_samplesInWindow >= BENCH_WINDOW_SAMPLES) {
      bench1_avgUsPerSample = bench1_sumUsInWindow / bench1_samplesInWindow;
      bench1_maxBurstUsOut  = bench1_maxBurstUs;
      uint32_t now  = time_us_32();
      uint32_t span = now - bench1_windowStart;
      bench1_samplesPerSec = span ? (uint32_t)(((uint64_t)bench1_samplesInWindow * 1000000ULL) / span) : 0;
      bench1_windowStart     = now;
      bench1_samplesInWindow = 0;
      bench1_sumUsInWindow   = 0;
      bench1_maxBurstUs      = 0;
    }
  }
#endif
